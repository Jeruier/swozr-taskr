...
class Console
{
    ...
    const VERB_QUIET = 0;
    const VERB_ERROR = 1; // default reporting on error
    const VERB_WARN  = 2;
    const VERB_INFO  = 3;
    const VERB_DEBUG = 4;
    const VERB_CRAZY = 5;
     ...
     * @param bool|int     $quit
     */
    public static function stdout($text, bool $nl = true, $quit = false)
    {
     ...
     * @param bool|int     $quit
     */
    public static function stderr($text, bool $nl = true, $quit = -2)
    {
        self::write($text, $nl, $quit, [
            'stream' => STDERR,
        ]);
    ...
    public static function log(string $msg, array $data = [], string $type = 'info', array $opts = [])
    {
        if (isset(self::LOG_LEVEL2TAG[$type])) {
            $type = Output::wrap(strtoupper($type), self::LOG_LEVEL2TAG[$type]);
        }
